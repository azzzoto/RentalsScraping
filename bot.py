...
async def set_search(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        starting_text = f"""Creazione di una ricerca automatizzata per appartamenti
           --> regione {r.capitalize()}, 
           --> provincia di {p.capitalize()}, 
           --> città di {c.capitalize()}"""
        await update.effective_message.reply_text(starting_text)
